#!/usr/bin/env bash
#SBATCH --job-name=udlm-training-clean-term-time-dependent-v2
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python -u -m main \
  ...
  sampling.steps=128 \
  wandb.name="owt_udlm_clean_loss_term_time_dependent-0-2" \
  hydra.run.dir="${PWD}/outputs/owt/udlm_clean_loss_term_time_dependent-0-2" \
  +training.clean_conf_lambda=0.02 \
  +training._clean_confidence_loss=1.0 \
