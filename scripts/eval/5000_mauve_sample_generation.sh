...
for greedy_step in 0 9; do
    ...
    python -u -m main \
        ...
        T=0 \
        sampling.num_sample_batches=157 \
        sampling.batch_size=32 \
        sampling.steps="$b_update" \
        sampling.use_cache=False \
        sampling.use_float64=False \
        eval.generated_samples_path="$PWD/greedy-tail-ablation-mauve-result/udlm-original-step-512-greedy-${greedy_step}.json" \
...
for greedy_step in 0 9; do
    ...
    python -u -m main \
        ...
        sampling.use_cache=False \
        sampling.use_float64=False \
        eval.generated_samples_path="$PWD/greedy-tail-ablation-mauve-result/udlm-clean-loss-term-time-dependent-v2-step-512-greedy-${greedy_step}.json" \
        +eval.generative_ppl_model_name_or_path="gpt2-large" \
        +sampling.noise_removal="greedy" \
        +sampling.noise_removal_steps="$greedy_step" \
