...
#SBATCH --job-name=udlm-owt-clean-term-v2-ablation
...
srun --environment=gidd -ul --cpu-bind=cores bash -c '
    ...
    python -u -m main \
        ...
        sampling.steps=512 \
        wandb.name="owt_udlm_clean_loss_term_time_dependent-v2-ablation" \
        hydra.run.dir="${PWD}/outputs/owt/udlm_clean_loss_term_time_dependent-v2-ablation" \
        +training.clean_conf_lambda=0.02 \
        +training.clean_conf_time_power=0.0 \
        +training.clean_conf_t_threshold=1.0 \
