...
#SBATCH --output=logs-train-slurm/original-udlm-%x-%j.out
#SBATCH --error=logs-train-slurm/original-udlm-%x-%j.err
...
srun -ul --cpu-bind=cores bash -c '
    ...
    python -u -m main \
        ...
        sampling.steps=512 \
        wandb.name="owt_udlm" \
        hydra.run.dir="${PWD}/outputs/owt/udlm" \
        +training.clean_conf_lambda=0.0 \
